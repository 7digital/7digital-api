...
APIBase.prototype.logger = new Logger(config);
...
APIBase.prototype.parseResponse = function(callback, response) {
	...
	console.log('in parsrRespons %j', this)
	parser.on('end', function(result) {
		console.log('hello: ' + this.format)
		if (this.format === 'json') {
	...
	APIBase.prototype.logger.log('parsing raw response');
...
APIBase.prototype.doGetRequest = function(action, data, callback) {
	...
	APIBase.prototype.logger.log('GET: ' + requestUrl);
	var host = this.host,
		headers = {
			'host': host,
	...
	apiRequest.on('response', function handleResponse(response) {
		...
		var that = this;
		console.log('response event' + this)
		...
		response.on("end", function endResponse() {
			FnUtils.bind(that, callback(responseBody));
		});
	});

	apiRequest.on('error', function logError(data) {
		APIBase.prototype.logger.log('Error fetching [' + url + ']. Body:\n' + data);
	});
...
APIBase.prototype.buildApiCall = function(apiCallDefinition) {
	...
	this.prototype[fnName] = function(params, callback) {
		var self = this;
		console.log('this in build: %j', self);
		this.doGetRequest(apiCallDefinition, params, 
			FnUtils.curry(this.parseResponse, callback));
	};
};
