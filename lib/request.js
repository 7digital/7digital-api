...
Request.prototype.makeSecureRequest = function (url, httpMethod, data,
	...
	var self = this,
		...
		token = is2Legged ? null : this.accesstoken,
		secret = is2Legged ? null : this.accesssecret;

	data = this.prepare(data);

	if (httpMethod === 'GET') {
		fullUrl = fullUrl + '?' + querystring.stringify(data);
		fullUrl = this.oauthClient.signUrl(fullUrl, token, secret);
		this.oauthClient.get(fullUrl, token, secret, callback);
	} else {
		//fullUrl = this.oauthClient.signUrl(fullUrl, token, secret);
