	...
	function cbWithDataAndResponse(err, data, response) {
		if (err) {
			...
			if (err.statusCode >= 500) {
				return callback(new ApiHttpError(response.statusCode, err.data, path));
			}

			if (err.statusCode >= 400 && typeof err.data === 'string' && /oauth/i.test(err.data)) {
				return callback(new OAuthError(err.data, err.data + ': ' + path));
			}
		}
