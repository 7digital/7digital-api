...
function dispatchSecure(url, httpMethod, requestData, authtype, hostInfo,
	...
	if (httpMethod === 'GET') {
		...
		fullUrl = actionhelper.template(fullUrl, requestData);
		fullUrl = fullUrl + '?' + querystring.stringify(requestData);
		logger.info(httpMethod + ': ' + fullUrl + ' (' + authtype + ' oauth)');

		oauthClient.get(fullUrl, token, secret, callbackWithDataAndResponse);
	} else {
		logger.info(httpMethod + ': ' + fullUrl + ' (' + authtype + ' oauth)');
		// FIXME: This will not work with the playlist API which expects a
		// JSON payload
