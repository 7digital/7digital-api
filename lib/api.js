 ...
 */
 function Api(schema, oauthkey /*optional*/, oauthsecret /*optional*/, format) {
	this.schema = schema;
	this.format = format;
	...
	for(var prop in schema.resources) {
		if (schema.resources.hasOwnProperty(prop)) {
			...
			var resourceConstructor = createResourceConstructor(resourceName, this.format); 

			resourceConstructor.prototype = new APIBase(
        resourceName, schema.host, schema.version, oauthkey, oauthsecret);
...
Api.buildFromFile = function(schemaPath, oauthkey /*optional*/, oauthsecret /*optional*/, format) {
	...
	return new Api(schema, oauthkey, oauthsecret, format);
};
