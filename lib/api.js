...
var APIRequest = require('./apirequest').APIRequest,
	sys = require('sys'),
	fs = require('fs'),
...
function Api(options) {
	var resourceName, resourceConstructor;

	this.schema = options.schema;
	this.format = options.format;
	this.logger = options.logger;

	function createResourceConstructor(resourceName, format, logger) {
	...
	for(var prop in options.schema.resources) {
		if (options.schema.resources.hasOwnProperty(prop)) {
			resourceName = options.schema.resources[prop].resource;
			resourceConstructor = createResourceConstructor(resourceName,
						this.format, options.logger);

			resourceConstructor.prototype = new APIRequest(
					options.schema.host, options.schema.version, 
					options.consumerkey, options.consumersecret, 
					options.logger);

			options.schema.resources[prop].actions.forEach(
			_.bind(resourceConstructor.prototype.buildApiCall,
					resourceConstructor));
