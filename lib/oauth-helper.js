...
function getAccessToken(options, callback) {
	...
	if (options.country) {
		...
		'POST', accessTokenUrl, { country: options.country }, null, null,
			function (error, data, response) {

			if( error ) return callback(error);

			var results= require('querystring').parse(data);
			var oauth_access_token= results["oauth_token"];
			delete results["oauth_token"];
			var oauth_access_token_secret= results["oauth_token_secret"];
			delete results["oauth_token_secret"];

			return callback(null, oauth_access_token, oauth_access_token_secret,
				results);
   });

	} else {
		oAuth.getOAuthAccessToken(options.requesttoken, options.requestsecret,
			callback);
	}
}
